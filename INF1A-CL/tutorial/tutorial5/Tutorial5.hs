...
reach1 a b = a `loves` b

reach2 :: Person -> Person -> Bool
reach2 a c = or [b `reach1` c | b <- people, a `reach1` b] 

reach3 :: Person -> Person -> Bool
reach3 a d = or [c `reach1` d | c <- people, a `reach2` c]

reach :: Person -> Person -> Int -> Bool
reach a c 1 = a `loves` c
reach a c n = or [reach b c 1 | b <- people, reach a b (n - 1)]

degreeSep :: Int -> Int
degreeSep n
    | and [reach a b n | a <- people, b <- people, a /= b] = n
    | otherwise = degreeSep (n+1)
...
check entered = 
  ...
  and [ or [entered i j k | i <- [(3*a-2)..(3*a)], j <- [(3*b-2)..(3*b)]]
      | a <- [1..3], b <- [1..3], k <- [1..9]]
