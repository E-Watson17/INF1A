...
wffToForm = canonical.toForm.toCNFList where
  toForm :: [[Wff a]] -> Form a
  toForm cs           = And (map toClause cs)
  toClause lits       = Or (map toLit lits)
  toLit (Not(V a) )   = N a
  toLit (V a)         = P a
  toLit _             = error "expected Literal"

prop_form_equiv wff = satisfiable wff == (dpll (wffToForm wff) /= [])
...
tseytinToForm p =
  let tt :: Eq a => Wff a -> [Clause (Wff a)]
      ...
      tt r@(a :&: b)   = [Or[P r, N a, N b], Or[N r, P a], Or[N r, P b]] ++ tt a ++ tt b
      tt r@(a :|: b)   = [Or[N r, P a, P b], Or[P r, N a], Or[P r, N b]] ++ tt a ++ tt b
      tt r@(a :->:  b) = [Or[N r, N a, P b], Or[P r, P a], Or[P r, N b]] ++ tt a ++ tt b
      tt r@(a :<->: b) = [Or[N r, N a, P b], Or[N r, P a, N b], Or[P r, P a, P b], Or[P r, N a, N b]] ++ tt a ++ tt b
...
prop_tseytin_equiv wff = satisfiable wff == (dpll (tseytinToForm wff) /= [])

prop_tseytin_bigger :: Wff Atom -> Bool
prop_tseytin_bigger wff = size (wffToForm wff) > 100 * size (tseytinToForm wff) where
  size :: Form a -> Int
  size (And cs) = length cs
