...
isBloodOrange (Orange variety _) = elem variety ["Tarocco", "Moro", "Sanguinello"]

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments = length . filter isBloodOrange

-- 3.
worms :: [Fruit] -> Int
worms = length . filter hasWorms where
  hasWorms :: Fruit -> Bool
  hasWorms (Orange _ _) = False
  hasWorms (Apple _ has) = has
...
equivalent wff1 wff2 = and [eval e wff1 == eval e wff2 | e <- envs (nub (atoms wff1 ++ atoms wff2))]
...
toNNF :: Wff a -> Wff a
toNNF wff = toNNF' (impElim wff) where
  toNNF' :: Wff a -> Wff a
  toNNF' (Not F)           = T
  toNNF' (Not T)           = F
...
isCNF wff
  | wff == F  = True
  | wff == T  = True
  | isNNF wff = isCNF' wff True
  | otherwise = False
    where
      isCNF' :: Eq a => Wff a -> Bool -> Bool
      isCNF' (V x) _         = True
      isCNF' (F) _           = False
      isCNF' (T) _           = False
      isCNF' (Not (V _)) _   = True
      isCNF' (Not _) _       = False
      isCNF' (p :&: q) True  = isCNF' p True && isCNF' q True
      isCNF' (p :&: q) False = False
      isCNF' (p :|: q) _     = isCNF' p False && isCNF' q False
      isCNF' _ _             = False
...
listsToCNF list
  | null list     = T
  | any null list = F 
  | otherwise     = foldr1 (:&:) (map (foldr1 (:|:)) list)
-- 14.
-- transform a CNF formula into a list of lists
listsFromCNF :: Eq a => Wff a -> [[Wff a]]
listsFromCNF wff = [[if state then Not (V val) else (V val) | (val, state) <- e] | e <- envs (atoms wff), not (eval e wff)]

-- 15.
-- transform an arbitrary formula into a list of lists
toCNFList :: Eq a => Wff a -> [[Wff a]]
toCNFList wff = cnf (toNNF wff) where
  cnf :: Eq a => Wff a -> [[Wff a]]
  cnf F             = [[]]
  cnf T             = []
  cnf (V x)         = [[V x]]
  cnf (Not (V x))   = [[Not (V x)]]
...
showWff e = showsPrec 0 e ""
  where
    ...
    showsPrec p (a :&: b)
      = showParen (p>3) (showsPrec 3 a .showSpace .showString "&"  . showSpace .showsPrec 3 b )
    showsPrec p (a :|: b)
      = showParen (p>2) (showsPrec 2 a .showSpace .showString "|"  . showSpace .showsPrec 2 b )
    showsPrec p (a :->: b)
      = showParen (p>1) (showsPrec 1 a .showSpace .showString "->" . showSpace .showsPrec 1 b )
    showsPrec p (a :<->: b)
      = showParen (p>0) (showsPrec 0 a .showSpace .showString "<->". showSpace .showsPrec 0 b )
    showsPrec _ (Not a) = 
